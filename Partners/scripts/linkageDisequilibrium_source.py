...
if __name__ == "__main__":
    ...
    fastafile = "/well/fraser/users/hqh588/pacbio_updated/data_donor/data/"+str(couple)+"/phyloscanner_output_" + str(window_len)+"_window/AlignedReads/" + filewindow
    sample_sequences = []
 ...
 #find positions of gaps: 
    ...
    df_linkage["type"]="donor"
        
    diversity["window"]=window
    diversity["couple"]=couple
    diversity["type"]="donor"  


    if not os.path.exists("/well/fraser/users/hqh588/pacbio_updated/output/diversity/diversity_donor_" + str(window_len) + "_"+couple+".csv"):
        diversity.to_csv("/well/fraser/users/hqh588/pacbio_updated/output/diversity/diversity_donor_" + str(window_len) + "_"+couple+".csv",index=False, header=False)
    else:
        diversity.to_csv("/well/fraser/users/hqh588/pacbio_updated/output/diversity/diversity_donor_" + str(window_len) + "_"+couple+".csv", mode='a', index=False, header=False)


    if not os.path.exists("/well/fraser/users/hqh588/pacbio_updated/output/linkage_recombination/linkage_donor_" + str(window_len) + "_"+couple+".csv"):
        df_linkage.to_csv("/well/fraser/users/hqh588/pacbio_updated/output/linkage_recombination/linkage_donor_" + str(window_len) + "_"+couple+".csv",index=False, header=False)

    else:
        df_linkage.to_csv("/well/fraser/users/hqh588/pacbio_updated/output/linkage_recombination/linkage_donor_" + str(window_len) + "_"+couple+".csv", mode='a', index=False, header=False)
